...
import com.moodmedia.storeportal.zimbra.content.ChainEmailContent;
...
public class ChainEmailUrl extends AUrl{
	...
	public void processRequest(HttpServletResponse response,
		...
		ChainEmailContent chainEmailContent = new ChainEmailContent();
		BufferedReader contentFromUrl;
		try {
			contentFromUrl = new BufferedReader(new InputStreamReader(chainEmailContent.getContentFromUrl(connection)));
			chainEmailContent.printContentToResponse(response, contentFromUrl);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
