package com.peter.motion_detection.detection;
...
public class RgbMotionDetection implements IMotionDetection {
	...
	private static final int mThreshold =1500; // Number of different pixels// (RGB)
	private static int[] mPrevious = null;
	private static int mPreviousWidth = 0;
	private static int mPreviousHeight = 0;
	public static  boolean movment=false;
	public static int pixtot=0;
	...
	//	Log.i(TAG, "TotDifferentPixels : >"+ totDifferentPixels +"  MThreshold: "+mThreshold + "  Size :"+ mPixelThreshold);
		boolean different = totDifferentPixels > mThreshold;
		
		//int size = 480* 800;
		int size = height * width;
		int percent = 100 / (size / totDifferentPixels);
		pixtot=totDifferentPixels;
		...
		//	Log.i(TAG, "RGB MOTION  MOTION DETECTION");
			
			movment=true;
		} else {
		//	Log.i(TAG, "RGB MOTION  NO MOTION DETECTION");
			movment=false;
		}
	...
	public boolean detect(int[] rgb, int width, int height) {
		...
		// Log.i(TAG, "Detection "+(aDetection-bDetection));
