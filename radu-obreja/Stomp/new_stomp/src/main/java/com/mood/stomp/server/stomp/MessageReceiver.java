		...
		try {
			...
			if(textParts.length == 1) {
				...
				// Prints help to console
					...
					stomp.sendLogMessage(HELPTEXT);
				}
				else if (command.equals(LIST_CONNECTED)) {
					Set<String> users = stomp.getQueues().keySet();
					...
					for(String user : users) {
						...
						QueueQuery qq = stomp.getQueueInfo(user);
			...
			// Commands with 1 parameter
				...
				if(command.equals(CREATE_QUEUE)){
					stomp.createQueue(parameter);
				}
				// Remove queue
				else if(command.equals(DELETE_QUEUE)) {
					stomp.removeQueue(parameter);
				}			
			}
			// Commands with 2 parameters
				...
				if(command.equals(SEND)) {
					stomp.sendMessage(parameter2, parameter1);				
				}
				// Creates user without password (validated against db)
				// with parameters username, role
				else if(command.equals(CREATE_USER)) {
					stomp.addListenRights(parameter2, parameter2);
					stomp.createQueue(parameter2);
					stomp.createUser(parameter1, null, parameter2);
				}
			}
				...
				// Sends a message to given user
				if(command.equals(SEND)) {
					stomp.sendMessage(parameter2 + " " + parameter3, parameter1);				
				}
				// Creates user with password (validated against server)
				// with parameters username, password, role
				if(command.equals(CREATE_USER)) {
					stomp.addListenRights(parameter3, parameter3);
					stomp.createQueue(parameter3);
					stomp.createUser(parameter1, parameter2, parameter3);
				}
			}
				...
				// Sends a message to given user
				if(command.equals(SEND)) {
					stomp.sendMessage(parameter2 + " " + parameter3 + " " + parameter4, parameter1);				
				}
			}
				...
				// Sends a message to given user
				if(command.equals(SEND)) {
					stomp.sendMessage(parameter2 + " " + parameter3 + 
